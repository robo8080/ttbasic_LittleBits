...
#define AIN_NUM   2
#define AOUT_NUM  2
int din_pin[DIN_NUM]   = {0};
int dout_pin[DOUT_NUM] = {1};
int ain_pin[AIN_NUM]   = {0, 1};
int aout_pin[AOUT_NUM] = {5, 9};

// Depending on device functions
// TO-DO Rewrite these functions to fit your machine
#define STR_EDITION "LittleBits"
...
//    short value2;
    ...
    if((value1 < 0)||(value1 >= DIN_NUM)) {
         err = ERR_SYNTAX;
        return 0;
    }
    if(digitalRead(din_pin[value1]) == LOW) {
      return 0;
    } else {
      return 1;
    }
...
short getain(){
    ...
    if((value1 < 0)||(value1 >= AIN_NUM)) {
         err = ERR_SYNTAX;
        return 0;
    }
    value2 = analogRead(ain_pin[value1]);
    return value2;
}
...
//  pwm[value1] = (float)value2 / 255.0f;
  analogWrite(aout_pin[value1],value2);
}
...
//    out[value1] = value2 & 0x01; 
    if(value2 == 0)
    {
      digitalWrite(dout_pin[value1],LOW);
    } else {
      digitalWrite(dout_pin[value1],HIGH);
    }
}
//----------------------------------------------------------------------
// Get value
...
void iprint() {
  ...
  while (*cip != I_SEMI && *cip != I_EOL) { //文末まで繰り返す
    ...
    case I_STR: //文字列の場合
      ...
      i = *cip++; //文字数を取得
      while (i--){ //文字数だけ繰り返す
        if((*cip == 0x5c)&&(i >= 3)) { //エスケープシーケンスの時の処理
            if(*(cip+1) == '0' && *(cip+2) == '3' && *(cip+3) == '3') {
                c_putch(0x1b);
                i -= 3; cip += 4;
            } else {
                c_putch(*cip++); //文字を表示
            }
        } else { //エスケープシーケンス以外の時の処理
            c_putch(*cip++); //文字を表示
        }
      }
      break; //打ち切る
